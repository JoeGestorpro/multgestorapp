...
async function ensureSubscriptionRecord({ companyId, module, plan, finance, status }, client) {
  ...
  const insertResult = await client.query(
    `INSERT INTO subscriptions (
       company_id, module_id, module_key, plan_id, plan_name, price,
       status, billing_cycle, next_due_date, started_at, gateway,
...
async function syncInvoice({ companyId, subscriptionId, finance, status, payload }, client) {
  ...
  const result = await client.query(
    `INSERT INTO invoices (
       company_id, subscription_id, gateway, external_invoice_id, external_charge_id,
       module_key, amount, status, due_at, paid_at, metadata, updated_at
     )
...
async function ensureFirstAccess(company, customer) {
  ...
  try {
    ...
    appLogger.error({ err: error }, '[billing-first-access-email-error]')
  }

  return generated
}

async function handleBillingProvisioning(eventPayload, context) {
  const { payment_gateway_event_id, provider, event_id, event_type, status, company: companyData, customer, finance, raw } = eventPayload
  const { subscriptionStatus, invoiceStatus } = status

  const client = await pool.connect()
  try {
    await client.query('BEGIN')

    const pgeCheck = await client.query(
      `SELECT processing_status, company_id, subscription_id
       FROM payment_gateway_events
       WHERE id = $1`,
      [payment_gateway_event_id]
    )

    if (pgeCheck.rows.length === 0 || pgeCheck.rows[0].processing_status !== 'pending') {
      await client.query('ROLLBACK')
      client.release()
      appLogger.info({ payment_gateway_event_id, status: pgeCheck.rows[0]?.processing_status }, 'Billing provisioning skipped (already processed or not found)')
      return
    }
    ...
    const module = await findModuleId(finance.moduleKey, client)
    const plan = await findPlan(module?.id || null, finance.planName, finance.billingCycle, client)
    ...
    if (company) {
      ...
      await createSubscriptionEvent(subscription.id, company.id, provider, { eventId: event_id, eventType: event_type }, raw || {}, client)
    }
    ...
    await client.query(
      `UPDATE payment_gateway_events
       SET processing_status = 'processed',
           ...
           processed_at = NOW()
       WHERE id = $1`,
      [payment_gateway_event_id, company?.id || null, subscription?.id || null]
    )
    ...
    appLogger.info({ payment_gateway_event_id, event_id, event_type }, 'Billing provisioning completed')
  } catch (error) {
    await client.query('ROLLBACK')
    appLogger.error({ err: error, payment_gateway_event_id }, 'Billing provisioning failed')
    throw error
  } finally {
    client.release()
  }
