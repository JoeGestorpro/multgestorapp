...
> **Atualizado:** 2026-06-19 · **Total:** 9 riscos ativos (4 P1, 4 P2, 1 resolvido)
...
### R-001 | Perda de todos os backups (catastrófico)

- **ID Auditoria:** A-002
- **Descrição:** Backup existe apenas no HD local (`C:\Users\Joefe\backups\`). Perda do equipamento = perda de todos os backups.
- **Probabilidade:** Média (HD pode falhar)
- **Impacto:** Catastrófico (perda de dados de cliente + operação)
- **Plano:** `ops/backup-external-copy` — cópia automática para Backblaze B2 pós-dump
- **Dono:** `next-task.md` (pending)
- **Tendência:** 🟡 Estável (plano pronto, aguarda ação humana)
- **Gate:** [[../gates/gate-producao|Gate Produção]] — critério #1
