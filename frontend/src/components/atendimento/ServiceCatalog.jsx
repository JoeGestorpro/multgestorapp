...
function ServiceCatalog({ services, products, onSelectItem, selectedItems }) {
  ...
  const catalogItems = useMemo(() => {
    ...
    return items
  }, [services, products, activeTab, search])

  const tabCounts = useMemo(() => {
    const all = [...services.map(s => ({ ...s, _type: 'service' })), ...products.map(p => ({ ...p, _type: 'product' }))]
    return {
      all: all.length,
  ...
  const isSelected = (item) => {
    return selectedItems.some(s => s.itemId === item.id)
  }
