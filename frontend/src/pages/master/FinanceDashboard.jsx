    ...
    async function loadDashboard() {
      ...
      try {
        const [
          ...
          api.get('/master/finance/mrr'),
          api.get('/master/finance/revenue-by-module'),
          api.get('/master/finance/revenue-by-gateway'),
          api.get('/master/finance/subscriptions', { params: { limit: 8 } }),
          api.get('/master/finance/events', { params: { limit: 8 } }),
          api.get('/master/finance/alerts')
        ])

        setOverview(overviewResponse.data.data || initialOverview)
        setMrrSeries(mrrResponse.data.data?.items || [])
        setRevenueByModule(moduleResponse.data.data?.items || [])
        setRevenueByGateway(gatewayResponse.data.data?.items || [])
        setSubscriptions(subscriptionsResponse.data.data?.items || [])
        setEvents(eventsResponse.data.data?.items || [])
        setAlerts(alertsResponse.data.data || { items: [], summary: { critical: 0, warning: 0 } })
      } catch (err) {
